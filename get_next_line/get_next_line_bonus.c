...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/05 22:27:50 by houamrha          #+#    #+#             */
/*   Updated: 2023/12/08 17:34:39 by houamrha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	buffer[fd] = read_data(fd, buffer[fd], &l);
	if (check_for_errors(l, buffer[fd]))
		return (free(buffer[fd]), buffer[fd] = NULL, NULL);
	line = set_to_line(buffer[fd]);
	if (!line)
